...
- `scripts/plot_hxmt_vs_gbm.py` — 跨卫星验证：HXMT/HE 重建光变 vs Fermi/GBM（扣本底、能量匹配、时间对齐）
...
### 跨卫星验证（GRB 260226A × Fermi/GBM）

GBM 在 200-900 keV 未饱和，提供独立参考光变曲线。验证要点：

**能量匹配**：GBM NaI ch72-124（199-909 keV）+ BGO b0 ch0-19（113-930 keV），对应 HXMT/HE 的 ~200-900 keV。

**时间对齐**：两个卫星使用不同的 MET 系统，转换到 UTC 时须注意闰秒：
- Fermi MET：TT 秒，epoch = 2001-01-01 00:00:00 UTC，用 astropy 转换
- HXMT MET：SI 秒，epoch = 2012-01-01 00:00:00 UTC，2012→2026 间有 3 个闰秒
- Python `datetime.total_seconds()` 不处理闰秒，直接用会差 3 秒
- GBM trigger（10:37:55.958 UTC）比 HXMT T=0（10:37:50.000 UTC）晚 5.958s
- 光行差 <47ms（两个 LEO 卫星），在 0.5s bin 下可忽略

**本底扣除**：用爆发前后的静默区间（默认 [-10,-2]+[60,80]s）估算常数本底并扣除，用净光变对比。
