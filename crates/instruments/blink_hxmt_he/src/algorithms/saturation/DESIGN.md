...
当前仍使用贪心+LIS 消歧。已知问题：三个独立 Box 在同一 GRB 的同一时段产生完全不相关的光变曲线（GRB 221009A T+249~T+268），说明 wrap 分配本质上是随机的。
...
1. Δstime>1 区间的可靠 wrap 消歧方法
2. FIFO 复位边界的精确检测与处理
3. Dead zone 事件的恢复（当前 ~4.6% 的事件因 ptime 落在 [500000, 524288) 区间而无法分配）
