...
fn process_chunk<E: Event>(chunk: &dyn Chunk<E = E>) -> Vec<Record<E>> {
    chunk
        .search()
        .iter()
        .filter_map(|signal| {
            let peak_time = signal.start + signal.delay + signal.bin_size_best / 2.0;
            ...
            .map(|lightning| {
                let is_associated = lightning.is_associated(
                    ...
                    uom::si::f64::Length::new::<uom::si::length::meter>(800_000.0),
                );
            ...
            let stop_best = (signal.start + signal.delay + signal.bin_size_best).to_utc();
            let peak = peak_time.to_utc();
            let duration_full = (signal.stop - signal.start).get::<uom::si::time::second>();
            let duration_best = signal.bin_size_best.get::<uom::si::time::second>();
            ...
            let events_unfiltered_full = signal
                ...
                .iter()
                .filter(|event| {
                    event.time().to_utc() >= start_full && event.time().to_utc() <= stop_full
                })
                .cloned()
                .collect::<Vec<_>>();
            let events_unfiltered_best = signal
                ...
                .iter()
                .filter(|event| {
                    event.time().to_utc() >= start_best && event.time().to_utc() <= stop_best
                })
                .cloned()
                .collect::<Vec<_>>();
            ...
            let associated_lightning_count = lightnings
                ...
                .count() as u32;
            let coincidence_probability = blink_lightning::algorithms::coincidence_prob(
                &position,
                TimeDelta::milliseconds(5),
                uom::si::f64::Length::new::<uom::si::length::meter>(800_000.0),
                TimeDelta::minutes(2),
            ...
            Some(Record {
                ...
                events: signal
                    ...
                    .iter()
                    .filter(|event| {
                        event.time().to_utc() >= start_full - TimeDelta::milliseconds(1)
                            && event.time().to_utc() <= stop_full + TimeDelta::milliseconds(1)
                    })
                    .cloned()
                    .collect(),
                ...
                altitude: position.state.altitude.get::<uom::si::length::meter>(),
                ...
                mean_solar_time: mean_solar_time(peak, position.state.longitude),
                apparent_solar_time: apparent_solar_time(peak, position.state.longitude),
    ...
    while let Some((time, satellite, detector)) = get_task(&conn, &worker) {
        match (satellite.as_str(), detector.as_str()) {
            ("HXMT", "HE") => {
                ...
                match chunk {
                    Ok(chunk) => {
                        let signals = process_chunk(&chunk);
            ...
            ("SVOM", "GRM") => {
                let chunk = blink_svom_grm::types::Chunk::from_epoch(&time);
                match chunk {
                    Ok(chunk) => {
                        let signals = process_chunk(&chunk);
                        for signal in signals {
