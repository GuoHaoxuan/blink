...
cursor.execute(
    """
    SELECT simultaneous_proportion_filtered_best, associated_lightning_count
    FROM signal
...
simultaneous_proportions = np.array([x[0] for x in signals])
simultaneous_proportions_lightning = np.array([x[0] for x in signals if x[1] > 0])
cursor.close()
conn.close()
...
plt.subplot(gs[1])
maxval = np.max(simultaneous_proportions)
minval = np.min(simultaneous_proportions)
bins = np.linspace(minval, maxval, 100)
plt.hist(simultaneous_proportions, bins=bins, color="C0", histtype="step")
plt.hist(simultaneous_proportions_lightning, bins=bins, color="C2", histtype="step")
# plt.xscale("log")
plt.yscale("log")
plt.xlim(minval, maxval)
plt.xlabel("Simultaneous Proportion")
plt.ylabel("Number")
...
plt.savefig(
    "hxmt-catalog/output/simultaneous.pdf", bbox_inches="tight", transparent=True
)
