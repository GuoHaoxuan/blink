...
cursor.execute(
    ...
    WHERE start_full < '2025-01-01'
        AND (false_positive_per_year <= 1e-5 OR (false_positive_per_year <= 1 AND associated_lightning_count > 0))
        AND lightnings = '[]'
    """
)
...
plt.savefig(
    "hxmt-catalog/output/geo-distribution-lightningless.pdf", bbox_inches="tight"
)
