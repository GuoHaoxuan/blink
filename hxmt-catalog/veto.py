...
cursor.execute(
    """
    SELECT veto_proportion_filtered_best, associated_lightning_count
    FROM signal
...
veto_proportions = np.array([x[0] for x in signals])
veto_proportions_lightning = np.array([x[0] for x in signals if x[1] > 0])
cursor.close()
conn.close()
...
plt.subplot(gs[1])
maxval = np.max(veto_proportions)
minval = np.min(veto_proportions)
bins = np.linspace(minval, maxval, 100)
plt.hist(veto_proportions, bins=bins, color="C0", histtype="step")
plt.hist(veto_proportions_lightning, bins=bins, color="C2", histtype="step")
# plt.xscale("log")
plt.yscale("log")
plt.xlim(minval, maxval)
plt.xlabel("Veto Proportion")
plt.ylabel("Number")
...
plt.savefig("hxmt-catalog/output/veto.pdf", bbox_inches="tight", transparent=True)
