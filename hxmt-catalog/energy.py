...
cursor.execute(
    """
    SELECT mean_energy_filtered_best, associated_lightning_count
    FROM signal
...
mean_energies = np.array([x[0] for x in signals])
mean_energies_lightning = np.array([x[0] for x in signals if x[1] > 0])
cursor.close()
conn.close()
...
plt.subplot(gs[1])
maxval = np.max(mean_energies)
minval = np.min(mean_energies)
bins = np.logspace(np.log10(minval), np.log10(maxval), 20)
plt.hist(mean_energies, bins=bins, color="C0", histtype="step")
plt.hist(mean_energies_lightning, bins=bins, color="C2", histtype="step")
plt.xscale("log")
plt.yscale("log")
plt.xlim(minval, maxval)
plt.xlabel("Mean Energy (Channel)")
plt.ylabel("Number")
...
plt.savefig("hxmt-catalog/output/energy.pdf", bbox_inches="tight", transparent=True)
