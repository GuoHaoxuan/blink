...
# 全部的
cursor.execute(
    "SELECT duration_best, 12, background, coincidence_probability, false_positive_per_year FROM signal WHERE start_full < '2025-01-01'"
)
...
# 闪电关联
cursor.execute(
    "SELECT duration_best, 12, background FROM signal WHERE start_full < '2025-01-01' AND associated_lightning_count > 0"
)
data = cursor.fetchall()
fp_years = [false_positive_per_year(row[2] * row[0], row[1], row[0]) for row in data]
fp_years = np.array(fp_years)
...
plt.xlabel("Expected Annual False Positive Under Poisson Assumption (12 Fake Number)")
plt.ylabel("Number")
...
plt.savefig("hxmt-catalog/output/fp-distribution3-fake-no.pdf", bbox_inches="tight")
