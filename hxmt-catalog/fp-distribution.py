...
log_bins = np.logspace(np.log10(min_fp), np.log10(max_fp), bins)
...
threshold_handle = plt.axvline(1e-3, color="C3", linewidth=0.5)


# 设置双y轴的标签
twinx.set_ylabel("Lightning Proportion")
# 设置双y的y轴范围
twinx.set_ylim(0, 1)
...
legend = plt.legend(
    ...
    labels=[
        "Lightning Free",
        "Lightning Free (Duration Filtered)",
        "Lightning Associated",
        "Lightning Associated (Duration Filtered)",
        "Lightning Proportion",
        "Lightning Proportion (Duration Filtered)",
        "Threshold",
    ],
    # frameon=True,
    # edgecolor="black",
