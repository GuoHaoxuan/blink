...
cursor.execute(
    """
    SELECT solar_zenith_angle, associated_lightning_count
    FROM signal
...
plt.subplot(gs[1])
plt.hist(angles, bins=np.arange(0, 182, 2), color="C0", histtype="step")
plt.hist(angles_lightning, bins=np.arange(0, 182, 2), color="C2", histtype="step")
plt.axvspan(
    0, 90, facecolor="C1", edgecolor="None", alpha=0.1, label="Daytime", zorder=-2
)
plt.axvspan(90, 180, facecolor="C0", edgecolor="None", alpha=0.1, zorder=-2)
plt.xlim(0, 180)
...
plt.savefig("hxmt-catalog/output/zenith.pdf", bbox_inches="tight", transparent=True)
