...
cursor.execute(
    """
    SELECT longitude, latitude, associated_lightning_count, start_full
    FROM signal
...
signals = [row for row in all_signals if parse(row[3]).month in [6, 7, 8]]
...
cm = 1 / 2.54
fig = plt.figure(figsize=(20 * cm, 14.2 * cm), dpi=1200)
...
gs = fig.add_gridspec(
    6,
    2,
    height_ratios=[0.7, 1, 3.5, 1.5, 1, 3.5],
    width_ratios=[12, 1],
    hspace=0,
    wspace=0,
)
...
legend_handles = [
    mpatches.Patch(facecolor="C0", edgecolor="None", label="TGFs"),
    mpatches.Patch(facecolor="C2", edgecolor="None", label="TGFs with Lightning"),
...
signals = [row for row in all_signals if parse(row[3]).month in [12, 1, 2]]
...
ax_map = fig.add_subplot(
    gs[2 + 3, 0], projection=ccrs.PlateCarree(central_longitude=150)
)
longitude_ax = fig.add_subplot(gs[1 + 3, 0])
latitude_ax = fig.add_subplot(gs[2 + 3, 1])
...
plt.savefig("hxmt-catalog/output/geo-distribution-season.pdf", bbox_inches="tight")
