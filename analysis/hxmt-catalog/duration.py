...
durations = np.array([x[0] for x in signals])
durations_lightning = np.array([x[0] for x in signals if x[1] > 0])
cursor.close()
conn.close()
...
plt.savefig("hxmt-catalog/output/duration.pdf", bbox_inches="tight", transparent=True)
