...
cursor.execute(
    ...
    SELECT
        ...
        orbit, lightnings
    FROM signal
    WHERE start_full = '2020-11-12T14:37:04.004140019Z' AND satellite = 'HXMT' AND detector = 'HE'
    """
)
...
ax_map.set_xticks(np.arange(90, 111, 10), crs=ccrs.PlateCarree())
ax_map.set_yticks(np.arange(30, 51, 10), crs=ccrs.PlateCarree())
ax_map.set_xticklabels(
    [
        f"{abs(int(x))}°{'W' if x < 0 else 'E' if x > 0 else ''}"
        for x in np.arange(90, 111, 10)
    ]
)
ax_map.set_yticklabels(
    [
        f"{abs(int(y))}°{'S' if y < 0 else 'N' if y > 0 else ''}"
        for y in np.arange(30, 51, 10)
    ]
)
...
plt.tight_layout()
plt.savefig("hxmt-catalog/output/detail4.pdf", bbox_inches="tight")
