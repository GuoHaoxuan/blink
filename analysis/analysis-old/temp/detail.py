...
cursor.execute(
    ...
    SELECT
        ...
        orbit, lightnings
    FROM signal
    WHERE start_full = '2024-06-26T01:35:07.999054014Z' AND satellite = 'SVOM' AND detector = 'GRM'
    """
)
...
lightnings = [lightning[0] for lightning in lightnings]
# print(lightnings)

longitudes_with_lightning = [lightning["lon"] for lightning in lightnings]
latitudes_with_lightning = [lightning["lat"] for lightning in lightnings]
ax_map.scatter(
...
# ax_detail.hist(
#     [
#         (dateutil_parse(event["time"]["time"]).timestamp() - start_full.timestamp())
#         * 1000
#         for event in events
#     ],
#     bins=bins,
...
# ax_detail.set_xlim(
#     (dateutil_parse(events[0]["time"]["time"]).timestamp() - start_full.timestamp())
#     * 1000,
#     (dateutil_parse(events[-1]["time"]["time"]).timestamp() - start_full.timestamp())
#     * 1000,
# )
