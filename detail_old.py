...
cursor.execute(
    ...
    SELECT
        ...
        orbit, lightnings
    FROM signal
    WHERE false_positive_per_year < 1e-2
    """
)
...
for signal in signals:
    ...
    gs = GridSpec(4, 1, height_ratios=[1, 4, 4, 4])

    # 创建各个子图
    ax_legend_light_curve = fig.add_subplot(gs[0, 0])
    ax_detail = fig.add_subplot(gs[1, 0])
    ax_100ms = fig.add_subplot(gs[2, 0])
    ax_1s = fig.add_subplot(gs[3, 0])
    ...
    ax_detail.set_xlim(
        ...
        (dateutil_parse(events[-1]["time"]).timestamp() - start_full.timestamp())
        * 1000,
    )
    ...
    legend_light_curve_handles = [
        ...
        mpatches.Patch(
            facecolor="C0", edgecolor="None", alpha=0.1, label="Full Duration"
        ),
        mpatches.Patch(
            facecolor="C2", edgecolor="None", alpha=0.2, label="Best Duration"
        ),
    ]
    ...
    plt.tight_layout()
    plt.savefig("output/{}.pdf".format(start_full), bbox_inches="tight")
