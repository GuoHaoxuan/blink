...
FIFO 复位后，MCU 清空 FIFO 并恢复正常读出。但 FIFO 中残留的第一个 SEC 事件是**陈旧的**——它的 anchor_met 反映的是 FIFO 拥塞期间的时间（比真实时间早 ~1s）。用这个陈旧锚点通过阈值法计算的事件时间会偏早约 1 个 WRAP_PERIOD。

**表现**：FIFO 复位后 ~100 包的事件被放到比正确位置早 ~1.05s 的位置。例如 Box B T+249 区域 ~9500 事件缺失（被错放到 T+248）。

### 核心思路：前向陈旧 + 后向新鲜

FIFO 复位后的第一个 SEC 是陈旧的（从 FIFO 缓冲中读出），但之后会出现一个**新鲜 SEC**（真正的整秒标记）。用新鲜 SEC 作为锚点反向修正之前的事件。
...
- 新鲜 SEC 判定：`!anchor_is_recent`（陈旧 SEC 在前 1-2 包内到达，仍属 "recent"）
...
- `biased_met`：陈旧锚点已计算的 MET（在 result 中）
- `met_base`, `met_alt`：后向 SEC 的两个候选（相差 ±WRAP_PERIOD）
- 选择与 `biased_met` 最接近的后向候选

**原理**：陈旧锚点和新鲜锚点有不同的 anchor_ptime，因此它们的 wrap 边界位置不同。当一个方向模糊时，另一个方向通常能给出有偏但方向正确的估计。
...
当陈旧锚点和新鲜锚点的 wrap 边界恰好重合时（两者的 anchor_ptime 差异使得同一批事件在两个方向上都处于 wrap 边界），前后对照无法消歧。表现为 `err_a ≈ err_b ≈ WRAP_PERIOD`。
...
### FIFO 复位后陈旧锚点 — 后向 SEC 修正
**根因**：FIFO 复位后首个 SEC 从 FIFO 缓冲读出，anchor_met 比真实时间早 ~1s。阈值法给出正确的 raw_delta 但基于错误的 anchor，导致事件偏早 ~1 WRAP_PERIOD。
**修复**：检测 FIFO 复位后的陈旧 SEC，缓冲后续事件。等新鲜 SEC 到达后用三阶段后向修正（置信度分类 → 前后对照 → 应用）。
...
5. **双重模糊**：当陈旧锚点和新鲜锚点的 wrap 边界重合时，前后对照无法消歧（Box B T+277/278）。
