...
静默丢数（Silent drops）是 FIFO 满时 FPGA 端阻塞写入、新事件被直接丢弃的现象。与 FIFO 复位不同，静默丢数没有任何硬件标记，理论上可以发生在任何时刻，包括单个 CCSDS 包内的 109 个事件之间。
...
2. 用短间隔（< 1ms）估算本底率 `λ = 1 / mean(dt_filtered)`
3. 对每个间隔计算泊松概率 `log₁₀(p) = -λ × dt / ln(10)`
4. 若 `log₁₀(p) < -10` 且 `dt < 10ms`，标记为静默丢数

### 8.3 放弃原因

对三个 GRB × 三个 Box 的检测结果进行人工逐图检查后，确认绝大部分为误报。根本问题：

1. **λ 在包内不稳定**：GRB 物理源率在毫秒尺度上变化显著。用包内最密集部分的事件率（短间隔均值）去评判率自然下降的区域，会将正常的稀疏区误判为丢数
2. **统计量不足**：单个包仅 109 个事件（~7ms 时间跨度），无法区分"源率变化"和"事件丢失"
3. **影响可忽略**：即使静默丢数真实发生，它发生在 FIFO 接近满的瞬间，随后立即触发 FIFO reset。单次 silent drop 丢失事件数（~几十个）远小于 FIFO reset gap 丢失的事件数（~数百至数千个）

### 8.4 结论

静默丢数作为硬件机制真实存在，但无法从数据中可靠检测，且对总事件损失的贡献可忽略。检测与重建代码已从 pipeline 中移除。文章中应描述此机制并给出上述定量论证。
...
根据饱和程度不同，分为两种可检测的情况，各有对应的检测和重建策略。静默丢数（FIFO 短暂满溢丢几个到几十个事件）作为物理机制存在，但无法可靠检测（详见第 8 节），不做处理。
...
输出 CSV 格式：`box,type,met,channel,pkt_idx,evt_idx`

- `EVT`：观测到的真实事件
- `FILL_GAP`：FIFO reset gap 补全的事件
- `FILL_DS`：深度饱和包级修正的事件
...
| `detect.rs` | 饱和检测 + 两种重建（FIFO reset gap / 深度饱和包级修正） |
