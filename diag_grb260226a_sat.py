#!/usr/bin/env python3
"""画 GRB 260226A: 1K光变 + 1B光变(Rust) + per-box 饱和区间

用法:
    HXMT_1B_DIR=data/1B cargo run -- 2026-02-26T10 --dump-times 446726278.000 60 2>/dev/null > dump_260226a.txt
    python3 diag_grb260226a_sat.py
"""
...
with open("dump_260226a.txt") as f:
    for line in f:
        line = line.strip()
        if not line:
            continue
...
# --- 2. 读取 1K ---
print("读取 1K...")
with fits.open("data/1K/Y202602/20260226-3179/HXMT_20260226T10_HE-Evt_FFFFFF_V1_1K.FITS") as f:
    time_1k = f[1].data["Time"]
m = (time_1k >= burst_met - 60) & (time_1k <= burst_met + 60)
time_1k = time_1k[m]
...
views = [
    (-10, 40, 0.1, "Full burst (100ms bins)"),
    (10, 30, 0.01, "Main burst (10ms bins)"),
    (14, 20, 0.002, "Peak detail (2ms bins)"),
]
...
for vi, (tmin, tmax, bw, title) in enumerate(views):
    ...
    ax_lc.set_title(f"GRB 260226A: {title}", fontsize=11)
...
plt.tight_layout()
plt.savefig("diag_grb260226a_sat.png", dpi=150)
print(f"Saved: diag_grb260226a_sat.png")
