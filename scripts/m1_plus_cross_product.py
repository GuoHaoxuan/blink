#!/usr/bin/env python3
"""M1 + κ·Sci·Sci_others post-hoc correction (Xiao 2020 cross-deadtime form).

Logic:
  Sci_obs ≈ Sci_pred_M1 + κ · Sci_pred_M1 · Sci_others
         = Sci_pred_M1 · (1 + κ · Sci_others)

Equivalently: residual_M1 = Sci_pred_M1 - Sci_obs ≈ -κ · Sci_pred_M1 · Sci_others.

Fit κ on high-Sci subset (no intercept):
   y = Sci_obs - Sci_pred_M1
   x = Sci_pred_M1 · Sci_others
   κ = Σ(x·y) / Σ(x²)

Apply globally:
   Sci_pred_final = Sci_pred_M1 · (1 + κ · Sci_others)
"""
...
def main():
    ...
    for box in "ABC":
        ...
        fits[box] = (b, one_plus_a - 1, beta, gamma)
    ...
    for box in "ABC":
        b, a, be, ga = fits[box]
        m = df["box"] == box
        pc = df.loc[m, "pho_rate"] - be*df.loc[m, "wide_rate"] - ga*df.loc[m, "large_rate"]
        df.loc[m, "sci_pred_m1"] = (pc - b) / (1 + a)
    df["resid_m1"] = df["sci_pred_m1"] - df["sci_rate"]

    # Fit κ per Box on high-Sci subset
    print(f"\n=== Cross-product κ fit on Sci > {KAPPA_FIT_LO} (no intercept) ===")
    box_kappa = {}
    for box in "ABC":
        sub = df[(df["box"] == box) & (df["sci_rate"] > KAPPA_FIT_LO)]
        x = sub["sci_pred_m1"].values * sub["sci_others_rate"].values
        y = sub["sci_rate"].values - sub["sci_pred_m1"].values  # -residual
        kappa = float((x * y).sum() / (x * x).sum())
        box_kappa[box] = kappa
        print(f"  Box {box}: κ = {kappa:+.6e}  (N={len(sub):,})")
    ...
    for box in "ABC":
        m = df["box"] == box
        k = box_kappa[box]
        df.loc[m, "sci_pred_final"] = df.loc[m, "sci_pred_m1"] * (1 + k * df.loc[m, "sci_others_rate"])
    df["resid_final"] = df["sci_pred_final"] - df["sci_rate"]

    print(f"\n=== RMS comparison ===")
    print(f"{'Box':>4s} {'RMS_M1':>9s} {'RMS_M1_main':>13s} {'RMS_final':>10s} {'RMS_final_main':>16s} "
          f"{'RMS_M1_hi':>10s} {'RMS_fin_hi':>11s}")
    for box in "ABC":
        sub = df[df["box"] == box]
        rms_m1   = float(np.sqrt(np.mean(sub["resid_m1"].values ** 2)))
        rms_m1_m = float(np.sqrt(np.mean(sub.loc[sub["sci_rate"] > MAIN_BAND_LO, "resid_m1"].values ** 2)))
        rms_fin  = float(np.sqrt(np.mean(sub["resid_final"].values ** 2)))
        rms_fin_m= float(np.sqrt(np.mean(sub.loc[sub["sci_rate"] > MAIN_BAND_LO, "resid_final"].values ** 2)))
        hi = sub["sci_rate"] > KAPPA_FIT_LO
        rms_m1_hi = float(np.sqrt(np.mean(sub.loc[hi, "resid_m1"].values ** 2)))
        rms_fin_hi= float(np.sqrt(np.mean(sub.loc[hi, "resid_final"].values ** 2)))
        print(f"  {box}    {rms_m1:>9.1f} {rms_m1_m:>13.1f} {rms_fin:>10.1f} {rms_fin_m:>16.1f} "
              f"{rms_m1_hi:>10.1f} {rms_fin_hi:>11.1f}")

    # Plot 1: corrected scatter
    ...
    for ax, box in zip(axes, "ABC"):
        ...
        ax.plot([SCI_MIN, SCI_MAX], [SCI_MIN, SCI_MAX], "r--", lw=1.8, zorder=6,
                label=f"y = x  (κ={box_kappa[box]:+.3e})")
    ...
    fig.suptitle(r"M1 × (1 + κ·Sci$_{\rm others}$): cross-detector dead-time correction (Xiao 2020 form)",
                 fontsize=12, y=0.995)
    out1 = OUT_DIR / "n_below_m1_cross.png"
    fig.savefig(out1, dpi=130, bbox_inches="tight")
    print(f"\nSaved: {out1}")

    # Plot 2: residual before/after
    fig, axes = plt.subplots(3, 1, figsize=(10, 12), sharex=True)
    for ax, box in zip(axes, "ABC"):
        sub = df[df["box"] == box]
        sci = sub["sci_rate"].values
        for col, label, color in [("resid_m1", "M1 (no correction)", "C0"),
                                   ("resid_final", "M1 × (1+κ·Sci_others)", "C3")]:
            resid = sub[col].values
            bins = np.logspace(np.log10(MAIN_BAND_LO), np.log10(4000.0), 40)
        ...
        ax.axvline(KAPPA_FIT_LO, color="gray", ls=":", lw=1, label=f"κ fit cut (Sci > {KAPPA_FIT_LO:.0f})")
        ax.set_xscale("log")
        ax.set_xlim(MAIN_BAND_LO, 4000)
        ax.set_xlabel("Sci obs [cnt/s/det]")
        ax.set_ylabel("residual = Sci_pred - Sci [cnt/s/det]")
        ax.set_title(f"Box {box}  κ = {box_kappa[box]:+.3e}")
        ax.legend(fontsize=9)
        ax.grid(alpha=0.3)
    fig.suptitle("Residual binned-median: M1 vs M1 × (1 + κ·Sci_others)",
                 fontsize=12)
    fig.tight_layout()
    out2 = OUT_DIR / "n_below_m1_cross_residual.png"
