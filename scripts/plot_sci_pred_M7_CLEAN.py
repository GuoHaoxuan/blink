#!/usr/bin/env python3
"""Sci_pred vs Sci_obs density plot using M7 (CLEAN band) — current best model.

M7: PHO = c0·Sci_pure + c1·Sci_ACD1 + cN·Sci_ACDN + β·Wide + γ·Large + b
   = c0·Sci + (c1-c0)·Sci_ACD1 + (cN-c0)·Sci_ACDN + β·Wide + γ·Large + b

Inverting for Sci_obs:
  Sci_pred = [PHO - (c1-c0)·Sci_ACD1 - (cN-c0)·Sci_ACDN - β·W - γ·L - b] / c0

If model perfect: Sci_pred ≈ Sci_obs (y=x line).
"""
...
def fit_m7(sub):
    ...
    return coef  # b, c_pure, c_ACD1, c_ACDN, β, γ


def main():
    df = load()

    print(f"\n=== M7 fit on CLEAN band [Sci ∈ [{SCI_LO_CLEAN}, {SCI_HI_CLEAN}]/det, "
    ...
    for box in "ABC":
        ...
        print(f"  Box {box} (N={mask.sum():,}): "
              f"b={b:+.1f}, c_pure={c0:.3f}, c_ACD1={c1:.3f}, c_ACDN={cN:.3f}, "
    ...
    for ax, box in zip(axes, "ABC"):
        ...
        ax.set_ylabel("Sci predicted (M7 inverted) [cnt/s/det]")
    ...
    fig.suptitle(f"M7 fit on CLEAN band (Sci ∈ [{SCI_LO_CLEAN:.0f}, {SCI_HI_CLEAN:.0f}]/det, "
                 f"box_rate < {BOX_RATE_CAP:.0f}); applied to full data.\n"
                 f"Sci_pred = [PHO − (c1−c0)·ACD1 − (cN−c0)·ACDN − β·W − γ·L − b] / c0",
                 fontsize=10, y=0.995)
    fig.subplots_adjust(left=0.10, right=0.90, top=0.96, bottom=0.04, hspace=0.18)
    out = OUT_DIR / "sci_pred_vs_obs_M7_CLEAN.png"
