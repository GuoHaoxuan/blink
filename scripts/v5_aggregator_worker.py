#!/usr/bin/env python3
"""v5 streaming aggregator — self-contained worker.

Processes one or more per-day per_sec parquet files. For each day:
  1. Compute |mlat| from Lat/Lon via aacgm grid.
  2. v2 unwrap (C=150) → resid_v2 = base_v2 - Sci_1s.
  3. Estimate s_det × 18 from |mlat| < 5° per (box, det).
  4. v5 unwrap with C_per_row = s_det · [1 + k · max(0, |mlat|-20)²], event-balance cap.
  5. Accumulate 2D histograms (sci vs base, sci vs resid_before, sci vs resid_after).

Output: one .npz per worker invocation with merged histograms + per-day s_det.

Usage:
    python3 v5_aggregator_worker.py \
        --parquet-glob 'per_sec_parquet/2020*.parquet' \
...
def process_day(df, interp, k_global):
    """Return dict of arrays + accumulators for one day."""
    ...
    # Per-det s_det at |mlat|<5°, fallback to global median if no data
    ...
    # If any s_det is missing (e.g., satellite always at high mlat that day), use
    # population median as fallback. Should be rare.
    pop_median = np.nanmedian(s_det_map)
    if not np.isfinite(pop_median):
        pop_median = 120.0  # safe default
    ...
    # blob/main counts (sci in 800-2500)
...
def main():
    ...
    p.add_argument("--parquet-glob", required=True, help="Glob for parquet files, e.g. 'per_sec_parquet/2020*.parquet'")
    p.add_argument("--aacgm-grid", required=True, help="Path to aacgm_grid_2020.npz")
    p.add_argument("--output", required=True, help="Output .npz path")
    p.add_argument("--k", type=float, default=0.00188, help="Global k (default 0.00188)")
    ...
    print(f"[worker] {len(files)} parquet files, output={args.output}", flush=True)
    ...
    print(f"[worker] aacgm loaded in {time.time()-t_init:.1f}s", flush=True)
    ...
    needed_cols = ["box", "det", "PHO", "Wide", "Large", "Sci_1s", "L_cycles", "Dt", "Lat", "Lon"]
    t0 = time.time()
    for i, pth in enumerate(files):
        date = os.path.basename(pth).split(".")[0]
        t_read = time.time()
        try:
            df = pd.read_parquet(pth, columns=needed_cols)
        except Exception as e:
            print(f"[worker] SKIP {date}: read failed {type(e).__name__}: {e}", flush=True)
            continue
        t_read_done = time.time()
        if len(df) == 0:
            print(f"[worker] SKIP {date}: empty", flush=True)
            continue
        try:
            r = process_day(df, interp, args.k)
        except Exception as e:
            print(f"[worker] FAIL {date}: {type(e).__name__}: {e}", flush=True)
            continue
        ...
        if (i + 1) % 50 == 0 or i == len(files) - 1 or i < 3:
            dt = time.time() - t0
            print(f"[worker] {i+1}/{len(files)} date={date} rows={len(df):,} "
                  f"read={t_read_done-t_read:.1f}s proc={t_proc_done-t_read_done:.1f}s "
    ...
    np.savez_compressed(
        ...
        n_capped=stats["n_capped"],
        n_blob=stats["n_blob"],
        n_main=stats["n_main"],
        n_below_yx=stats["n_below_yx"],
    )
    print(f"[worker] DONE in {time.time()-t0:.1f}s -> {args.output}", flush=True)
    print(f"[worker] stats: total={stats['n_total']:,} clean={stats['n_clean']:,} "
          f"valid={stats['n_valid']:,} below_yx={stats['n_below_yx']:,} "
          f"blob/main={stats['n_blob']}/{stats['n_main']}", flush=True)
