...
def fit_v10(sub):
    """V10: 9 free params + intercept = 10 per det. Returns (coef[10], err[10])."""
    X = np.column_stack([np.ones(len(sub)),
                          sub["scipure_rate"], sub["acd_rate"],
...
def main():
    ...
    # ===== V10 per-det fit =====
    print("\nFitting V10 (9 params + b per det)...")
    v10_fits = {}
    v10_errs = {}
    fallback = []
    for box in "ABC":
    ...
    # ----- Title + header -----
    formula = (r"$\mathbf{PHO_i = c_{pure}\!\cdot\!Sci_{pure,i} + c_{ACD}\!\cdot\!Sci_{ACD,i} + \beta\!\cdot\!Wide_i + \gamma\!\cdot\!Large_i + "
               r"c_{pure}'\!\cdot\!Sci_{pure,js} + c_{ACD}'\!\cdot\!Sci_{ACD,js} + \beta'\!\cdot\!Wide_{js} + \gamma'\!\cdot\!Large_{js} + b}$"
               + "\n   (advisor's 2nd formula — cross-detector j-sum from other 5 dets in box)")
    fig.text(0.05, 0.945, formula, fontsize=11)
    fig.text(0.05, 0.27,
              "Per-det fit coefficients ± 1σ  "
              "(top row: own-det channels w/ V8 ghost;  bottom row: cross-det j-sum coefs)",
              fontsize=12, weight='bold')
    fig.suptitle("V10 cross-detector model — training + 260226A burst overlay   "
                 "(9 params + b per det = 180 total)",
                 fontsize=14, y=0.98)

    out = OUT_DIR / "sci_pred_M7merged_perdet_V10_crossdet.png"
