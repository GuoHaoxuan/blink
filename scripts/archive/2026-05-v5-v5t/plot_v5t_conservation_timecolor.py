...
def main():
    ...
    for f in files:
        ...
        for rg in np.unique(np.linspace(0, n_rg - 1, args.rowgroups_per_file).astype(int)):
            ...
            sci, rec, ty = process_rows(df, interp, calib)
            ok = np.isfinite(sci) & np.isfinite(rec) & (sci > 0)
            sci_all.append(sci[ok]); rec_all.append(rec[ok]); ty_all.append(ty[ok])
        print(f"  {os.path.basename(f)}: {sum(len(a) for a in sci_all):,} pts")

    sci = np.concatenate(sci_all); rec = np.concatenate(rec_all); ty = np.concatenate(ty_all)
    if len(sci) > args.max_points:
        sel = np.random.RandomState(0).choice(len(sci), args.max_points, replace=False)
        sci, rec, ty = sci[sel], rec[sel], ty[sel]
    resid = rec - sci
    year = 2017.48 + ty   # calendar year (t0 = 2017-06-22)
    ...
    o = np.random.RandomState(1).permutation(int(m1.sum()))
    sc1 = ax1.scatter(sci[m1][o], rec[m1][o], c=year[m1][o], cmap="turbo",
                      vmin=2017, vmax=2026.5, s=2, alpha=0.5,
                      rasterized=True, edgecolor="none")
    cax1 = inset_axes(ax1, width="30%", height="2.5%", loc="upper left", borderpad=2.5)
    cb1 = fig.colorbar(sc1, cax=cax1, orientation="horizontal")
    cb1.set_label("observation year", fontsize=9); cb1.ax.tick_params(labelsize=8)
    ...
    o2 = np.random.RandomState(2).permutation(int(m2.sum()))
    ax2.scatter(sci[m2][o2], resid[m2][o2], c=year[m2][o2], cmap="turbo",
                vmin=2017, vmax=2026.5, s=2, alpha=0.5,
                rasterized=True, edgecolor="none")
    ...
    o3 = np.random.RandomState(3).permutation(int(m3.sum()))
    ax3.scatter(sci[m3][o3], frac[m3][o3], c=year[m3][o3], cmap="turbo",
                vmin=2017, vmax=2026.5, s=2, alpha=0.5,
                rasterized=True, edgecolor="none")
