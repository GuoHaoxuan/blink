...
def fit_m7_merged(sub, constrained=True):
    """constrained=True: fix c_pure=1, γ=1; fit only (b, c_ACD, β).
       constrained=False: V8 baseline 5-param fit."""
    if constrained:
...
def fit_m7_merged_with_err(sub, constrained=True):
    """Return (coef, std_err) — 5-vectors. constrained=True fixes c_pure=γ=1
    (zero std_err for those two positions)."""
    if constrained:
        X = np.column_stack([np.ones(len(sub)), sub["acd_rate"], sub["wide_rate"]])
        y = sub["pho_rate"].values - sub["scipure_rate"].values - sub["large_rate"].values
        c3, *_ = np.linalg.lstsq(X, y, rcond=None)
...
def main():
    ...
    formula = (r"$\mathbf{PHO = Sci_{pure} + c_{ACD}\cdot Sci_{ACD} "
               ...
               r"   ($c_{pure} = \gamma = 1$ fixed)")
    fig.text(0.05, 0.94, formula, fontsize=13)
    fig.text(0.05, 0.255,
              r"Per-detector fit coefficients $\pm$ 1$\sigma$ OLS standard error  "
              r"(V8 ghost: $c_{pure}, \gamma$ free; constrained: $c_{pure}=\gamma=1$)",
              fontsize=12, weight='bold')

    fig.suptitle("M7 merged-ACD per-detector with $c_{pure}=\\gamma=1$ fixed — "
                 "training + 260226A burst overlay   (3 params × 18 dets = 54 params)",
                 fontsize=14, y=0.975)

    out = OUT_DIR / "sci_pred_M7merged_perdet_cpure1_gamma1.png"
    fig.savefig(out, dpi=240, bbox_inches="tight")
    print(f"\nSaved: {out}")
    desktop = Path.home() / "Desktop" / "sci_pred_M7merged_perdet_cpure1_gamma1.png"
