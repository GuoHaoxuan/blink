#!/usr/bin/env python3
"""Residual structure plot WITH per-det multiplicative β on full Sci_pred.

residual = β · [(PHO − Large)·(1−dt)/L − Wide/L] − Sci_1s

β fitted per-det by least squares (same fit as plot_pho_simple_perdet_beta.py).

Diagnostic question: does β (multiplicative shift) leave a clean residual or
introduce structure? Since the original residual is mostly constant +120 cnt/s,
a β fit will TRANSLATE the cloud in log-log but introduce a SLOPE in linear
residual (because multiplying by β<1 over-scales high-Sci predictions).

Output: plots/pho_residual_structure_beta.png (new file, doesn't overwrite).
"""
...
DEFAULT_OUT = Path("plots/pho_residual_structure_beta.png")
...
def make_plot(df: pd.DataFrame, betas: dict, out_path: Path) -> None:
    ...
    beta_row = df["_beta_val"].astype("float32")
    df["sci_obs"] = df["Sci_1s"].astype("float32")
    df["residual"] = (beta_row * base - df["sci_obs"]).astype("float32")
    ...
    for row, box in enumerate("ABC"):
        for col in range(6):
            ...
            if len(sub) > 100:
                ...
                fit_coefs.append((box, col, b_fit, m_fit, len(sub), betas[(box, col)]))
                ax.text(0.04, 0.96,
                         f"β = {betas[(box, col)]:.3f}\nintercept = {b_fit:+.1f}\nslope k = {m_fit:+.4f}\nN = {len(sub):,}",
    ...
    fig.suptitle(
        ...
        "Residual vs Sci_1s observed   per (box, det)   (per-det multiplicative β fitted, 2020-H1 clean, PSD anomaly excluded)",
    ...
    print("\nPer (box, det) — β + post-β residual fit:")
    print(f"  {'(box,det)':<10}{'β':>8}{'intercept (cnt/s)':>20}{'slope k':>12}{'N':>10}")
    for box, det, b, m, n, beta in fit_coefs:
        print(f"  {box}-{det}      {beta:>7.3f}{b:>+20.2f}{m:>+12.5f}{n:>10,}")
