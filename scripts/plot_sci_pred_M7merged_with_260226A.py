#!/usr/bin/env python3
"""M7 with merged ACD channel (per-box coefficients), 260226A overlaid."""
...
def load_260226A():
    """Load 260226A engineering + 1B Sci."""
    rows = []
    ...
    # ACD ratios from typical observations (~22% total ACD coincidence rate)
...
def main():
    ...
    # PHO = c_pure·Sci_pure + c_ACD·Sci_ACD + β·Wide + γ·Large + b
    #     = c_pure·Sci + (c_ACD - c_pure)·Sci_ACD + β·Wide + γ·Large + b
    # → Sci_pred = (PHO - (c_ACD - c_pure)·Sci_ACD - β·Wide - γ·Large - b) / c_pure
    ...
    for ax, box in zip(axes, "ABC"):
        ...
        # 260226A overlay (only burst bins: FILL_GAP > 0)
        ...
        ax.set_title(f"Box {box}  M7-merged: c_pure={c[1]:.2f}, c_ACD={c[2]:.2f}, "
                      f"β={c[3]:.2f}, γ={c[4]:.2f}, b={c[0]:+.0f}",
                      fontsize=9)
    ...
    fig.suptitle("M7 with merged ACD channel (5 params per box) — 260226A overlaid\n"
                 + formula,
                 fontsize=14, y=1.005)
    out = OUT_DIR / "sci_pred_M7merged_with_260226A.png"
