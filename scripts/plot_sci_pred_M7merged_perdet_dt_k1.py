#!/usr/bin/env python3
"""V8 + deadtime correction with FIXED k=1 (user's original hypothesis).

Model:  PHO · (1 − dt/L) = c_pure·Sci_pure + c_ACD·Sci_ACD + β·Wide + γ·Large + b
                                ↑ k=1 fixed, not free-fit

Self-consistent inversion with per-(det, sec) local r = Sci_ACD / Sci:

  Sci_pred = [PHO·(1 − dt/L) − β·Wide − γ·Large − b] / [c_pure·(1−r) + c_ACD·r]

Compared to V8 (no dt correction), this tests whether the user's physically
motivated k=1 correction improves burst validation."""
...
def load_training():
    dtype = {"date": "string", "box": "category", "met_sec": "int64",
             ...
             "Dt": "int32",                              # for k=1 dt correction
    ...
    df["dt_frac"] = df["Dt"].astype("float32") / df["L_cycles"]   # for k=1 dt correction
...
def fit_m7_merged(sub, dt_corrected=True):
    """5 params: b, c_pure, c_ACD, β, γ.
       dt_corrected=True  → target = PHO·(1 - dt/L)   (k=1 fixed)
       dt_corrected=False → target = PHO              (V8 baseline for compare)"""
    ...
    y = (sub["pho_rate"].values * (1.0 - sub["dt_frac"].values)
         if dt_corrected else sub["pho_rate"].values)
    coef, *_ = np.linalg.lstsq(X, y, rcond=None)
    return coef


def fit_m7_merged_with_err(sub, dt_corrected=True):
    """Return (coef, std_err). dt_corrected switches target between PHO·(1-dt/L) and PHO."""
    X = np.column_stack([np.ones(len(sub)), sub["scipure_rate"], sub["acd_rate"],
                          sub["wide_rate"], sub["large_rate"]])
    y = (sub["pho_rate"].values * (1.0 - sub["dt_frac"].values)
         if dt_corrected else sub["pho_rate"].values)
...
def load_260226A():
    ...
    for box, code in [("A","0766"),("B","1009"),("C","1781")]:
        ...
        for det_local in range(6):
            ...
            dt = d[f"DeadTime_PHODet_{det_g}"].astype(float)   # for k=1 dt correction
    ...
    eng["dt_frac"] = eng["dt_cyc"] / eng["length_cyc"]   # for k=1 dt correction
...
def apply_coefs(df, coef_map):
    """Self-consistent Sci_pred with k=1 dt correction:

       Sci_pred = [PHO·(1 - dt/L) - β·Wide - γ·Large - b] / [c_pure·(1-r) + c_ACD·r]

    where r = Sci_ACD / Sci (per-(det, sec) local ratio).
    ...
    denom = c0 * (1.0 - r) + cA * r
    pho_corrected = df["pho_rate"].values * (1.0 - df["dt_frac"].values)
    pred = (pho_corrected - bet*df["wide_rate"].values
...
def main():
    ...
    # Then per-(box, det) fits (with errors) — both k=1 dt-corrected and V8 baseline
    fits_dict, errs_dict = {}, {}
    fits_v8,   errs_v8   = {}, {}
    fallback_keys = []
    ...
    for box in "ABC":
        for det in range(6):
            ...
            if N < N_MIN_PERDET:
                ...
                # V8 fallback also = box-level no-dt fit
                sub_box = train[((train["box"]==box)
                                  & (train["sci_rate"] >= SCI_LO_CLEAN)
                                  & (train["sci_rate"] < SCI_HI_CLEAN)
                                  & (train["group_rate"] < BOX_RATE_CAP))]
                fits_v8[(box, det)] = fit_m7_merged(sub_box, dt_corrected=False)
                ...
                c, e = fit_m7_merged_with_err(train[mask], dt_corrected=True)
                fits_dict[(box, det)] = c
                errs_dict[(box, det)] = e
                # V8 baseline (no dt) for visual comparison
                c8, e8 = fit_m7_merged_with_err(train[mask], dt_corrected=False)
                fits_v8[(box, det)] = c8
                errs_v8[(box, det)] = e8
    ...
    # Implied PHO prediction = RHS_fit / (1 - dt/L)  (since target was PHO·(1-dt/L))
    ...
    # rms_perdet[k]   = k=1 dt-corrected per-det RMS (this script's model)
    # rms_perdet_v8[k]= V8 no-dt per-det RMS (baseline for comparison)
    # Compute V8 implied PHO prediction (no dt: target was PHO directly)
    ...
    pho_pred_v8 = (b8 + c08*train["scipure_rate"].values
                    ...
                    + gm8*train["large_rate"].values)   # V8: no lf_inv
    rms_perdet = {}
    rms_perdet_v8 = {}
    for box in "ABC":
        for det in range(6):
    ...
    for row, box in enumerate("ABC"):
        for col in range(6):
            ...
            # RMS annotation (per-det, PHO units) + delta vs V8 baseline
    ...
    for p_idx, (ax, name) in enumerate(zip(axes3, param_names)):
        for i, (b, _) in enumerate(det_order):
            # Thin connecting line from V8 ghost to k=1 marker (same det)
            ax.plot([coef_v8a[i, p_idx], coef_arr[i, p_idx]],
                    [y_pos[i], y_pos[i]],
                    color=box_color[b], lw=0.7, alpha=0.35, zorder=2)
            # V8 baseline (no dt) — light ghost
            ...
            # k=1 dt-corrected — bold foreground
    ...
    legend_handles2 = [
        plt.Line2D([], [], marker='|', color="#d62728", lw=0,
                   markersize=10, markeredgewidth=1.8, label="Box A (k=1)"),
        plt.Line2D([], [], marker='|', color="#2ca02c", lw=0,
                   markersize=10, markeredgewidth=1.8, label="Box B (k=1)"),
        plt.Line2D([], [], marker='|', color="#1f77b4", lw=0,
                   markersize=10, markeredgewidth=1.8, label="Box C (k=1)"),
        plt.Line2D([], [], marker='|', color='gray', lw=0, alpha=0.30,
                   markersize=7, markeredgewidth=1.0,
                   label="V8 no-dt (ghost)"),
        plt.Line2D([], [], color='gray', lw=0.7, alpha=0.6,
                   label="shift V8 → k=1"),
    ]
    ...
    # ----- Section titles -----
    formula = (r"$\mathbf{PHO \cdot (1 - dt/L) = c_{pure}\cdot Sci_{pure} + c_{ACD}\cdot Sci_{ACD} "
    ...
    fig.suptitle("M7 merged-ACD per-detector model + dt correction (k=1 fixed) — "
                 "training + 260226A burst overlay   (5 params × 18 dets = 90 params)",
                 fontsize=14, y=0.975)

    out = OUT_DIR / "sci_pred_M7merged_perdet_dt_k1.png"
    fig.savefig(out, dpi=240, bbox_inches="tight")
    print(f"\nSaved: {out}")
    desktop = Path.home() / "Desktop" / "sci_pred_M7merged_perdet_dt_k1.png"
